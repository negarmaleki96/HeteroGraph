...
p_net_rshares_train = []
p_abs_rshares_train = []
p_vote_rshares_train = []
p_author_rewards_train = []
p_author_reputation_train = []
...
c_net_rshares_train = []
c_abs_rshares_train = []
c_vote_rshares_train = []
c_author_rewards_train = []
c_author_reputation_train = []

def checkKey(dict, key):
    if key in dict.keys():
        None
    else:
        G_train.nodes[i]['author_reputation'] = 0

for i in range(len(G_train.nodes())):
    if G_train.nodes()[i]['otype']=='post':
        p_otype_train.append(1)
        checkKey(G_train.nodes[i],'author_reputation')
        p_created_train.append(int(G_train.nodes[i]['created']))
        p_last_payout_train.append(int(G_train.nodes[i]['last_payout']))
        p_payout_train.append(int(G_train.nodes[i]['payout']))     # [] removed to have y=[] instead of y=[ ,1]
        p_net_rshares_train.append(int(G_train.nodes[i]['net_rshares']))
        p_abs_rshares_train.append(int(G_train.nodes[i]['abs_rshares']))
        p_vote_rshares_train.append(int(G_train.nodes[i]['vote_rshares']))
        p_author_rewards_train.append(int(G_train.nodes[i]['author_rewards']))
        p_author_reputation_train.append(int(G_train.nodes[i]['author_reputation']))
        ...
        checkKey(G_train.nodes[i],'author_reputation')
        c_created_train.append(int(G_train.nodes[i]['created']))
        c_last_payout_train.append(int(G_train.nodes[i]['last_payout']))
        c_payout_train.append(int(G_train.nodes[i]['payout']))
        c_net_rshares_train.append(int(G_train.nodes[i]['net_rshares']))
        c_abs_rshares_train.append(int(G_train.nodes[i]['abs_rshares']))
        c_vote_rshares_train.append(int(G_train.nodes[i]['vote_rshares']))
        c_author_rewards_train.append(int(G_train.nodes[i]['author_rewards']))
        c_author_reputation_train.append(int(G_train.nodes[i]['author_reputation']))
...
p_abs_rshares_val = []
p_vote_rshares_val = []
p_author_rewards_val = []
p_author_reputation_val = []
...
c_abs_rshares_val = []
c_vote_rshares_val = []
c_author_rewards_val = []
c_author_reputation_val = []

def checkKey(dict, key):
    if key in dict.keys():
        None
    else:
        G_val.nodes[i]['author_reputation'] = 0

for i in range(len(G_val.nodes())):
    if G_val.nodes()[i]['otype']=='post':
        p_otype_val.append(1)
        checkKey(G_val.nodes[i],'author_reputation')
        p_created_val.append(int(G_val.nodes[i]['created']))
        p_last_payout_val.append(int(G_val.nodes[i]['last_payout']))
        p_payout_val.append(int(G_val.nodes[i]['payout']))     # [] removed to have y=[] instead of y=[ ,1]
        p_net_rshares_val.append(int(G_val.nodes[i]['net_rshares']))
        p_abs_rshares_val.append(int(G_val.nodes[i]['abs_rshares']))
        p_vote_rshares_val.append(int(G_val.nodes[i]['vote_rshares']))
        p_author_rewards_val.append(int(G_val.nodes[i]['author_rewards']))
        p_author_reputation_val.append(int(G_val.nodes[i]['author_reputation']))
        ...
        checkKey(G_val.nodes[i],'author_reputation')
        c_created_val.append(int(G_val.nodes[i]['created']))
        c_last_payout_val.append(int(G_val.nodes[i]['last_payout']))
        c_payout_val.append(int(G_val.nodes[i]['payout']))
        c_net_rshares_val.append(int(G_val.nodes[i]['net_rshares']))
        c_abs_rshares_val.append(int(G_val.nodes[i]['abs_rshares']))
        c_vote_rshares_val.append(int(G_val.nodes[i]['vote_rshares']))
        c_author_rewards_val.append(int(G_val.nodes[i]['author_rewards']))
        c_author_reputation_val.append(int(G_val.nodes[i]['author_reputation']))
...
p_abs_rshares_test = []
p_vote_rshares_test = []
p_author_rewards_test = []
p_author_reputation_test = []
...
c_abs_rshares_test = []
c_vote_rshares_test = []
c_author_rewards_test = []
c_author_reputation_test = []

def checkKey(dict, key):
    if key in dict.keys():
        None
    else:
        G_test.nodes[i]['author_reputation'] = 0

for i in range(len(G_test.nodes())):
    if G_test.nodes()[i]['otype']=='post':
        p_otype_test.append(1)
        checkKey(G_test.nodes[i],'author_reputation')
        p_created_test.append(int(G_test.nodes[i]['created']))
        p_last_payout_test.append(int(G_test.nodes[i]['last_payout']))
        p_payout_test.append(int(G_test.nodes[i]['payout']))     # [] removed to have y=[] instead of y=[ ,1]
        p_net_rshares_test.append(int(G_test.nodes[i]['net_rshares']))
        p_abs_rshares_test.append(int(G_test.nodes[i]['abs_rshares']))
        p_vote_rshares_test.append(int(G_test.nodes[i]['vote_rshares']))
        p_author_rewards_test.append(int(G_test.nodes[i]['author_rewards']))
        p_author_reputation_test.append(int(G_test.nodes[i]['author_reputation']))
        ...
        checkKey(G_test.nodes[i],'author_reputation')
        c_created_test.append(int(G_test.nodes[i]['created']))
        c_last_payout_test.append(int(G_test.nodes[i]['last_payout']))
        c_payout_test.append(int(G_test.nodes[i]['payout']))
        c_net_rshares_test.append(int(G_test.nodes[i]['net_rshares']))
        c_abs_rshares_test.append(int(G_test.nodes[i]['abs_rshares']))
        c_vote_rshares_test.append(int(G_test.nodes[i]['vote_rshares']))
        c_author_rewards_test.append(int(G_test.nodes[i]['author_rewards']))
        c_author_reputation_test.append(int(G_test.nodes[i]['author_reputation']))

# creaing dataframe for features
# train
post_df_train = pd.concat([pd.DataFrame(p_otype_train, columns=['otype']), pd.DataFrame(p_created_train, columns=['created']),
                     pd.DataFrame(p_last_payout_train, columns=['last_payout']), pd.DataFrame(p_payout_train, columns=['payout']),
                     pd.DataFrame(p_net_rshares_train, columns=['net_rshares']), pd.DataFrame(p_abs_rshares_train, columns=['abs_rshares']),
                     pd.DataFrame(p_vote_rshares_train, columns=['vote_rshares']), pd.DataFrame(p_author_rewards_train, columns=['author_rewards']),
                     pd.DataFrame(p_author_reputation_train, columns=['author_reputation'])], axis=1, ignore_index=True)
post_df_train.columns = ['otype', 'created', 'last_payout', 'payout', 'net_rshares', 'abs_rshares', 'vote_rshares',
                   'author_rewards', 'author_reputation']
comment_df_train = pd.concat([pd.DataFrame(c_otype_train, columns=['otype']), pd.DataFrame(c_created_train, columns=['created']),
                     pd.DataFrame(c_last_payout_train, columns=['last_payout']), pd.DataFrame(c_payout_train, columns=['payout']),
                     pd.DataFrame(c_net_rshares_train, columns=['net_rshares']), pd.DataFrame(c_abs_rshares_train, columns=['abs_rshares']),
                     pd.DataFrame(c_vote_rshares_train, columns=['vote_rshares']), pd.DataFrame(c_author_rewards_train, columns=['author_rewards']),
                     pd.DataFrame(c_author_reputation_train, columns=['author_reputation'])], axis=1, ignore_index=True)
comment_df_train.columns = ['otype', 'created', 'last_payout', 'payout', 'net_rshares', 'abs_rshares', 'vote_rshares',
                      'author_rewards', 'author_reputation']
...
# validation
post_df_val = pd.concat([pd.DataFrame(p_otype_val, columns=['otype']), pd.DataFrame(p_created_val, columns=['created']),
                     pd.DataFrame(p_last_payout_val, columns=['last_payout']), pd.DataFrame(p_payout_val, columns=['payout']),
                     pd.DataFrame(p_net_rshares_val, columns=['net_rshares']), pd.DataFrame(p_abs_rshares_val, columns=['abs_rshares']),
                     pd.DataFrame(p_vote_rshares_val, columns=['vote_rshares']), pd.DataFrame(p_author_rewards_val, columns=['author_rewards']),
                     pd.DataFrame(p_author_reputation_val, columns=['author_reputation'])], axis=1, ignore_index=True)
post_df_val.columns = ['otype', 'created', 'last_payout', 'payout', 'net_rshares', 'abs_rshares', 'vote_rshares',
                   'author_rewards', 'author_reputation']
comment_df_val = pd.concat([pd.DataFrame(c_otype_val, columns=['otype']), pd.DataFrame(c_created_val, columns=['created']),
                     pd.DataFrame(c_last_payout_val, columns=['last_payout']), pd.DataFrame(c_payout_val, columns=['payout']),
                     pd.DataFrame(c_net_rshares_val, columns=['net_rshares']), pd.DataFrame(c_abs_rshares_val, columns=['abs_rshares']),
                     pd.DataFrame(c_vote_rshares_val, columns=['vote_rshares']), pd.DataFrame(c_author_rewards_val, columns=['author_rewards']),
                     pd.DataFrame(c_author_reputation_val, columns=['author_reputation'])], axis=1, ignore_index=True)
comment_df_val.columns = ['otype', 'created', 'last_payout', 'payout', 'net_rshares', 'abs_rshares', 'vote_rshares',
                      'author_rewards', 'author_reputation']
...
# test
post_df_test = pd.concat([pd.DataFrame(p_otype_test, columns=['otype']), pd.DataFrame(p_created_test, columns=['created']),
                     pd.DataFrame(p_last_payout_test, columns=['last_payout']), pd.DataFrame(p_payout_test, columns=['payout']),
                     pd.DataFrame(p_net_rshares_test, columns=['net_rshares']), pd.DataFrame(p_abs_rshares_test, columns=['abs_rshares']),
                     pd.DataFrame(p_vote_rshares_test, columns=['vote_rshares']), pd.DataFrame(p_author_rewards_test, columns=['author_rewards']),
                     pd.DataFrame(p_author_reputation_test, columns=['author_reputation'])], axis=1, ignore_index=True)
post_df_test.columns = ['otype', 'created', 'last_payout', 'payout', 'net_rshares', 'abs_rshares', 'vote_rshares',
                   'author_rewards', 'author_reputation']
comment_df_test = pd.concat([pd.DataFrame(c_otype_test, columns=['otype']), pd.DataFrame(c_created_test, columns=['created']),
                     pd.DataFrame(c_last_payout_test, columns=['last_payout']), pd.DataFrame(c_payout_test, columns=['payout']),
                     pd.DataFrame(c_net_rshares_test, columns=['net_rshares']), pd.DataFrame(c_abs_rshares_test, columns=['abs_rshares']),
                     pd.DataFrame(c_vote_rshares_test, columns=['vote_rshares']), pd.DataFrame(c_author_rewards_test, columns=['author_rewards']),
                     pd.DataFrame(c_author_reputation_test, columns=['author_reputation'])], axis=1, ignore_index=True)
comment_df_test.columns = ['otype', 'created', 'last_payout', 'payout', 'net_rshares', 'abs_rshares', 'vote_rshares',
                      'author_rewards', 'author_reputation']
...
df_scaled_train = std_scaler_train.fit_transform(dataset_train.to_numpy())
df_scaled_train = pd.DataFrame(df_scaled_train, columns=['otype', 'created', 'last_payout', 'payout', 'net_rshares',
                                             'abs_rshares', 'vote_rshares', 'author_rewards', 'author_reputation'])

# validation
std_scaler_val = StandardScaler()
df_scaled_val = std_scaler_val.fit_transform(dataset_val.to_numpy())
df_scaled_val = pd.DataFrame(df_scaled_val, columns=['otype', 'created', 'last_payout', 'payout', 'net_rshares',
                                             'abs_rshares', 'vote_rshares', 'author_rewards', 'author_reputation'])

# test
std_scaler_test = StandardScaler()

df_scaled_test = std_scaler_test.fit_transform(dataset_test.to_numpy())
df_scaled_test = pd.DataFrame(df_scaled_test, columns=['otype', 'created', 'last_payout', 'payout', 'net_rshares',
                                             'abs_rshares', 'vote_rshares', 'author_rewards', 'author_reputation'])

# type of nodes dataframe
...
# creating instance of one-hot-encoder for type of node
# train
...
for i in range(post_df_train.shape[0]):
    ...
    p_created_train.append([df_scaled_train.iloc[i, 1]])
    p_last_payout_train.append([df_scaled_train.iloc[i, 2]])
    p_payout_train.append(dataset_train.iloc[i, 3])
    p_net_rshares_train.append([df_scaled_train.iloc[i, 4]])
    p_abs_rshares_train.append([df_scaled_train.iloc[i, 5]])
    p_vote_rshares_train.append([df_scaled_train.iloc[i, 6]])
    p_author_rewards_train.append([df_scaled_train.iloc[i, 7]])
    p_author_reputation_train.append([df_scaled_train.iloc[i, 8]])

for i in range(len(df_scaled_train),len(df_otype_train)):
...
for i in range(post_df_train.shape[0], len(df_scaled_train)):
    ...
    c_created_train.append([df_scaled_train.iloc[i, 1]])
    c_last_payout_train.append([df_scaled_train.iloc[i, 2]])
    c_payout_train.append([df_scaled_train.iloc[i, 3]])
    c_net_rshares_train.append([df_scaled_train.iloc[i, 4]])
    c_abs_rshares_train.append([df_scaled_train.iloc[i, 5]])
    c_vote_rshares_train.append([df_scaled_train.iloc[i, 6]])
    c_author_rewards_train.append([df_scaled_train.iloc[i, 7]])
    c_author_reputation_train.append([df_scaled_train.iloc[i, 8]])
...
for i in range(post_df_val.shape[0]):
    ...
    p_created_val.append([df_scaled_val.iloc[i, 1]])
    p_last_payout_val.append([df_scaled_val.iloc[i, 2]])
    p_payout_val.append(dataset_val.iloc[i, 3])
    p_net_rshares_val.append([df_scaled_val.iloc[i, 4]])
    p_abs_rshares_val.append([df_scaled_val.iloc[i, 5]])
    p_vote_rshares_val.append([df_scaled_val.iloc[i, 6]])
    p_author_rewards_val.append([df_scaled_val.iloc[i, 7]])
    p_author_reputation_val.append([df_scaled_val.iloc[i, 8]])

for i in range(len(df_scaled_val),len(df_otype_val)):
...
for i in range(post_df_val.shape[0], len(df_scaled_val)):
    ...
    c_created_val.append([df_scaled_val.iloc[i, 1]])
    c_last_payout_val.append([df_scaled_val.iloc[i, 2]])
    c_payout_val.append([df_scaled_val.iloc[i, 3]])
    c_net_rshares_val.append([df_scaled_val.iloc[i, 4]])
    c_abs_rshares_val.append([df_scaled_val.iloc[i, 5]])
    c_vote_rshares_val.append([df_scaled_val.iloc[i, 6]])
    c_author_rewards_val.append([df_scaled_val.iloc[i, 7]])
    c_author_reputation_val.append([df_scaled_val.iloc[i, 8]])
...
for i in range(post_df_test.shape[0]):
    ...
    p_created_test.append([df_scaled_test.iloc[i, 1]])
    p_last_payout_test.append([df_scaled_test.iloc[i, 2]])
    p_payout_test.append(dataset_test.iloc[i, 3])
    p_net_rshares_test.append([df_scaled_test.iloc[i, 4]])
    p_abs_rshares_test.append([df_scaled_test.iloc[i, 5]])
    p_vote_rshares_test.append([df_scaled_test.iloc[i, 6]])
    p_author_rewards_test.append([df_scaled_test.iloc[i, 7]])
    p_author_reputation_test.append([df_scaled_test.iloc[i, 8]])

for i in range(len(df_scaled_test),len(df_otype_test)):
...
for i in range(post_df_test.shape[0], len(df_scaled_test)):
    ...
    c_created_test.append([df_scaled_test.iloc[i, 1]])
    c_last_payout_test.append([df_scaled_test.iloc[i, 2]])
    c_payout_test.append([df_scaled_test.iloc[i, 3]])
    c_net_rshares_test.append([df_scaled_test.iloc[i, 4]])
    c_abs_rshares_test.append([df_scaled_test.iloc[i, 5]])
    c_vote_rshares_test.append([df_scaled_test.iloc[i, 6]])
    c_author_rewards_test.append([df_scaled_test.iloc[i, 7]])
    c_author_reputation_test.append([df_scaled_test.iloc[i, 8]])
...
p_created_train = torch.tensor(p_created_train).float()
p_payout_train = torch.tensor(p_payout_train).float()
p_last_payout_train = torch.tensor(p_last_payout_train).float()
p_net_rshares_train = torch.tensor(p_net_rshares_train).float()
p_abs_rshares_train = torch.tensor(p_abs_rshares_train).float()
p_vote_rshares_train = torch.tensor(p_vote_rshares_train).float()
p_author_rewards_train = torch.tensor(p_author_rewards_train).float()
p_author_reputation_train = torch.tensor(p_author_reputation_train).float()
...
p_created_val = torch.tensor(p_created_val).float()
p_payout_val = torch.tensor(p_payout_val).float()
p_last_payout_val = torch.tensor(p_last_payout_val).float()
p_net_rshares_val = torch.tensor(p_net_rshares_val).float()
p_abs_rshares_val = torch.tensor(p_abs_rshares_val).float()
p_vote_rshares_val = torch.tensor(p_vote_rshares_val).float()
p_author_rewards_val = torch.tensor(p_author_rewards_val).float()
p_author_reputation_val = torch.tensor(p_author_reputation_val).float()
...
p_created_test = torch.tensor(p_created_test).float()
p_payout_test = torch.tensor(p_payout_test).float()
p_last_payout_test = torch.tensor(p_last_payout_test).float()
p_net_rshares_test = torch.tensor(p_net_rshares_test).float()
p_abs_rshares_test = torch.tensor(p_abs_rshares_test).float()
p_vote_rshares_test = torch.tensor(p_vote_rshares_test).float()
p_author_rewards_test = torch.tensor(p_author_rewards_test).float()
p_author_reputation_test = torch.tensor(p_author_reputation_test).float()
...
c_created_train = torch.tensor(c_created_train).float()
c_last_payout_train = torch.tensor(c_last_payout_train).float()
c_payout_train = torch.tensor(c_payout_train).float()
c_net_rshares_train = torch.tensor(c_net_rshares_train).float()
c_abs_rshares_train = torch.tensor(c_abs_rshares_train).float()
c_vote_rshares_train = torch.tensor(c_vote_rshares_train).float()
c_author_rewards_train = torch.tensor(c_author_rewards_train).float()
c_author_reputation_train = torch.tensor(c_author_reputation_train).float()
...
c_created_val = torch.tensor(c_created_val).float()
c_last_payout_val = torch.tensor(c_last_payout_val).float()
c_payout_val = torch.tensor(c_payout_val).float()
c_net_rshares_val = torch.tensor(c_net_rshares_val).float()
c_abs_rshares_val = torch.tensor(c_abs_rshares_val).float()
c_vote_rshares_val = torch.tensor(c_vote_rshares_val).float()
c_author_rewards_val = torch.tensor(c_author_rewards_val).float()
c_author_reputation_val = torch.tensor(c_author_reputation_val).float()
...
c_created_test = torch.tensor(c_created_test).float()
c_last_payout_test = torch.tensor(c_last_payout_test).float()
c_payout_test = torch.tensor(c_payout_test).float()
c_net_rshares_test = torch.tensor(c_net_rshares_test).float()
c_abs_rshares_test = torch.tensor(c_abs_rshares_test).float()
c_vote_rshares_test = torch.tensor(c_vote_rshares_test).float()
c_author_rewards_test = torch.tensor(c_author_rewards_test).float()
c_author_reputation_test = torch.tensor(c_author_reputation_test).float()
...
# Create edge of graph for pytorch
# train
...
# train
# Post features
data_train['post'].x = torch.cat([p_otype_u_train, p_otype_p_train, p_otype_c_train, p_net_rshares_train,
                                  p_abs_rshares_train, p_vote_rshares_train, p_author_rewards_train,
                                  p_author_reputation_train], dim=-1)
...
# Comment features
data_train['comment'].x = torch.cat([c_otype_u_train, c_otype_p_train, c_otype_c_train, c_payout_train,
                                     c_net_rshares_train, c_abs_rshares_train, c_vote_rshares_train,
                                     c_author_rewards_train, c_author_reputation_train], dim=-1)

# validation
# Post features
data_val['post'].x = torch.cat([p_otype_u_val, p_otype_p_val, p_otype_c_val, p_net_rshares_val, p_abs_rshares_val,
                                p_vote_rshares_val, p_author_rewards_val, p_author_reputation_val], dim=-1)
...
# Comment features
data_val['comment'].x = torch.cat([c_otype_u_val, c_otype_p_val, c_otype_c_val, c_payout_val, c_net_rshares_val,
                                   c_abs_rshares_val, c_vote_rshares_val, c_author_rewards_val,
                                   c_author_reputation_val], dim=-1)

# test
# Post features
data_test['post'].x = torch.cat([p_otype_u_test, p_otype_p_test, p_otype_c_test, p_net_rshares_test,
                                 p_abs_rshares_test, p_vote_rshares_test, p_author_rewards_test,
                                 p_author_reputation_test], dim=-1)
...
# Comment features
data_test['comment'].x = torch.cat([c_otype_u_test, c_otype_p_test, c_otype_c_test, c_payout_test, c_net_rshares_test,
                                    c_abs_rshares_test, c_vote_rshares_test, c_author_rewards_test,
                                    c_author_reputation_test], dim=-1)
...
data_train['user', 'vote', 'comment'].edge_index = vote_c_train
...
data_train['user', 'vote', 'comment'].edge_attr = torch.cat([v_c_otype_train, v_c_time_train], dim=-1)
...
data_val['user', 'vote', 'comment'].edge_index = vote_c_val
...
data_val['user', 'vote', 'comment'].edge_attr = torch.cat([v_c_otype_val, v_c_time_val], dim=-1)
...
data_test['user', 'vote', 'comment'].edge_index = vote_c_test
...
data_test['user', 'vote', 'comment'].edge_attr = torch.cat([v_c_otype_test, v_c_time_test], dim=-1)
...
model = GAT(hidden_channels=128, out_channels=3)
...
pd.DataFrame(y_predicted_test['post'].argmax(dim=1)).to_csv("~/model1_3class_testpred.csv")
pd.DataFrame(data_test['post'].y.type(torch.LongTensor)).to_csv("~/model1_3class_testactual.csv")
model1_3C_pred = pd.read_csv("~/model1_3class_testpred.csv")
model1_3C_act = pd.read_csv("~/model1_3class_testactual.csv")
cr = classification_report(model1_3C_act.iloc[:,0], model1_3C_pred.iloc[:,0])
